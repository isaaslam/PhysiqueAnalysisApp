...
def analyse_image(image_url):
    ...
    ideal_shoulder_to_waist = 2.0
    shoulder_score = min((shoulder_to_waist_ratio / ideal_shoulder_to_waist) * 10, 10) if shoulder_to_waist_ratio else 0

    # Leg-to-torso ratio scoring (ideal ~1.45, stricter tolerance ±0.35)
    ideal_leg_to_torso = 1.45
    leg_score = max(0, min(10, (1 - abs(leg_to_torso_ratio - ideal_leg_to_torso) / 0.35) * 10)) if leg_to_torso_ratio else 0

    # --- Final proportion score ---
    proportion_score = round((shoulder_score * 0.7 + leg_score * 0.3), 2)
